<template>
  <el-form label-width="100px"> 
    ...
    <el-form-item label="SPU销售属性">
        <!-- 展示销售属性的下拉菜单,将选中的:value值存储到saleAttrIdAndValueName中-->
        <el-select v-model="saleAttrIdAndValueName" :placeholder="unSelectSaleAttr.length ? `还未选择${unSelectSaleAttr.length}个` : '无'" style="width: 150px;">
            <el-option :value="`${item.id}:${item.name}`" v-for="(item, index) in unSelectSaleAttr" :key="item.id" :label="item.name"></el-option>
        </el-select>
        <!--通过判断是否选择属性的值，来确定添加按钮是否可用-->
...
import type {SaleAttrValue,Trademark, SpuData, AllTradeMark, SpuHasImg, SpuImg, SaleAttr,HasSaleAttr, HasSaleAttrResponseData} from '@/api/product/spu/type';
...
const initHasSpuData = async (spu: SpuData) => {
    //存储已有的SPU对象,将来在模板中展示
    spuParams.value = spu;
    //spu:即为父组件传递过来的已有的SPU对象[不完整]，需要后面其他请求补全
    //获取全部品牌的数据
