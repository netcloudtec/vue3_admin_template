<template>
  <div>
    <!-- 三级分类，通过:scene的值控制三级分类下拉菜单是否灰化 -->
    <Category :scene="scene"></Category>
    <!-- 通过style的margin属性控制和Category组件的距离 -->
    <el-card style="margin: 10px 0px">
      <!-- v-if|v-show:都可以实现显示与隐藏 -->
      <div v-show="scene == 0">
            <!--根据三级分类是否有id来判断是否可以添加SPU  -->
            ...
            <el-table  style="margin: 10px 0px" border :data="records">
                <!--type="index"代表此列式序号列，width="80px"限制此列的宽度  -->
                <el-table-column label="序号" type="index" align="center" width="80px"></el-table-column>
                <el-table-column label="SPU名称" prop="spuName"></el-table-column>
                <!--show-overflow-tooltip文字太对会隐藏 -->
            ...
            <!-- 分页器 -->
            <!-- @current-change="getHasSpu"页面发生变化的时候，发送一次请求，@size-change="sizeChange"分页器下拉菜单发生变化的时候触发 -->
            <el-pagination v-model:current-page="pageNo" v-model:page-size="pageSize" :page-sizes="[3, 5, 7, 9]"
                        :background="true" layout="prev, pager, next, jumper,->,sizes,total" :total="total"
                        @current-change="getHasSpu" @size-change="changeSize"/>   
        </div>
        <!-- 添加SPU|修改SPU子组件
            @changeScene绑定自定义事件，通过子组件给父组件传递值，修改父组件的scene值进行场景的切换
            ref="spu" 父组件获取子组件的实例-->
